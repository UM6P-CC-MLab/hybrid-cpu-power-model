#!/system/bin/sh
# phase2_combined.sh - Phase 2: symmetric rho sweep, all clusters on
#
# For each rho in a list, pin each cluster to the nearest DVFS step
# corresponding to rho * maxFreq_i, run stress on all cores, measure.
#
# Usage:
#   sh phase2_combined.sh [rho1,rho2,...]
#
# Default rho list: 0.20, 0.35, 0.50, 0.60, 0.70, 0.80, 0.90, 1.00
#
# Env overrides:
...
# --- Parse args -------------------------------------------------------------
DEFAULT_RHOS="0.20 0.35 0.50 0.60 0.70 0.80 0.90 1.00"
...
echo "========================================================================"
echo " PHASE 2: Symmetric rho sweep (all clusters active)"
echo "========================================================================"
...
# All clusters online for this phase
online_all
for cluster in $CLUSTERS; do
...
# --- Sweep rho --------------------------------------------------------------
for rho in $RHOS; do
  # Compute target freq for each cluster
  ...
  # Actual achieved rho (for logging; the CSV computes this per-sample too)
  ...
  echo ""
  echo "┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓"
  echo "┃  rho=$rho  little=${fL}kHz(ρ=${aL})  big=${fB}kHz(ρ=${aB})  prime=${fP}kHz(ρ=${aP})"
  echo "┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛"
  ...
  while [ "$rep" -le "$NUM_REPS" ]; do
    ...
    # Thermal wait may have offlined cores; re-pin
    online_all
    pin_frequency little "$fL"
    pin_frequency big    "$fB"
    pin_frequency prime  "$fP"
